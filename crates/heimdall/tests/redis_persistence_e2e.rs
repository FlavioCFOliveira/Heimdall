...
fn redis_with_password_blocking(password: &str) -> Option<(u16, impl Drop)> {
    use testcontainers::{core::WaitFor, runners::SyncRunner, GenericImage, ImageExt as _};
...
fn zone_hset_carries_documented_field_shape() {
    use heimdall_runtime::store::zone_store::{ZoneRrset, write_zone};
    use heimdall_runtime::store::{RrsetPayload, zone_key};
    ...
    rt.block_on(async {
        ...
        assert_eq!(
            &a_value[1..5], &[0x01, 0x02, 0x03, 0x04],
...
fn zone_rename_swap_live_key_reflects_v2_staging_key_absent() {
    use heimdall_runtime::store::zone_store::{ZoneRrset, write_zone, get_rrset};
    use heimdall_runtime::store::{RrsetPayload, zone_staging_key};
    ...
    rt.block_on(async {
        ...
        let make_rrsets = |ttl: u32| {
            vec![
                ZoneRrset {
                    ...
                    qtype: 1, qclass: 1,
                    rrset: RrsetPayload { ttl, rdata: vec![vec![1, 1, 1, 1]] },
                },
                ZoneRrset {
                    owner: "sub.atomic-rename-test.".to_owned(),
                    qtype: 1, qclass: 1,
                    rrset: RrsetPayload { ttl, rdata: vec![vec![2, 2, 2, 2]] },
                },
            ]
        };

        write_zone(&store, fqdn, &make_rrsets(100)).await.expect("write v1");
        ...
        write_zone(&store, fqdn, &make_rrsets(200)).await.expect("write v2");

        // Both apex and sub must reflect v2.
        let apex = get_rrset(&store, fqdn, "atomic-rename-test.", 1, 1)
            .await.expect("get apex v2").expect("apex must be present");
        assert_eq!(apex.ttl, 200, "(ii) apex TTL must be 200 after v2; got {}", apex.ttl);

        let sub = get_rrset(&store, fqdn, "sub.atomic-rename-test.", 1, 1)
            .await.expect("get sub v2").expect("sub must be present");
        assert_eq!(sub.ttl, 200, "(ii) sub TTL must be 200 after v2; got {}", sub.ttl);
...
fn cache_namespace_isolation_recursive_invisible_from_forwarder() {
    use heimdall_runtime::store::cache_store::{write_cache, read_cache};
    use heimdall_runtime::store::{CacheEntry, CacheNamespace, DnssecOutcome, RrsetPayload};
    ...
    rt.block_on(async {
        ...
        let rec_entry = CacheEntry {
            ...
            rrset: RrsetPayload { ttl: 60, rdata: vec![vec![10, 0, 0, 1]] },
        };

        // Write to Recursive namespace.
        write_cache(&store, CacheNamespace::Recursive, owner, qtype, qclass, &rec_entry, 60)
            .await
        ...
        let read_rec = read_cache(&store, CacheNamespace::Recursive, owner, qtype, qclass)
            .await.expect("read recursive").expect("recursive entry must be present");
        assert_eq!(read_rec.rrset.ttl, 60, "(iii) Recursive entry TTL must be 60");

        // Must NOT be readable from Forwarder.
        let read_fwd = read_cache(&store, CacheNamespace::Forwarder, owner, qtype, qclass)
            .await.expect("read forwarder (must not error)");
        assert!(
            read_fwd.is_none(),
        ...
        let fwd_entry = CacheEntry {
            ...
            rrset: RrsetPayload { ttl: 120, rdata: vec![vec![10, 0, 0, 2]] },
        };
        write_cache(&store, CacheNamespace::Forwarder, owner, qtype, qclass, &fwd_entry, 120)
            .await.expect("write forwarder cache entry");
        ...
        let read_rec_after = read_cache(&store, CacheNamespace::Recursive, owner, qtype, qclass)
            .await.expect("re-read recursive after forwarder write")
        ...
        let read_fwd_after = read_cache(&store, CacheNamespace::Forwarder, owner, qtype, qclass)
            .await.expect("read forwarder after write")
