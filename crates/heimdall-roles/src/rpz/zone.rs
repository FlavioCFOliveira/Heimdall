...
use crate::rpz::{
    action::RpzAction,
    trie::{CidrTrie, NsdnameMatcher, QnameTrie},
...
impl PolicyZone {
    ...
    pub fn check_response_ip(&self, addrs: &[IpAddr]) -> Option<RpzAction> {
        addrs
            .iter()
            .find_map(|&ip| self.response_ip.lookup(ip).cloned())
    }
    ...
    pub fn check_nsip(&self, ns_addrs: &[IpAddr]) -> Option<RpzAction> {
        ns_addrs
            .iter()
            .find_map(|&ip| self.nsip.lookup(ip).cloned())
    }
    ...
    pub fn check_nsdname(&self, ns_names: &[Name]) -> Option<RpzAction> {
        ns_names
            .iter()
            .find_map(|n| self.nsdname.lookup(n).cloned())
    }
}
...
mod tests {
    ...
    fn insert_and_check_client_ip() {
        ...
        assert_eq!(
            z.check_client_ip(IpAddr::V4(Ipv4Addr::new(11, 0, 0, 1))),
            None
        );
    }
