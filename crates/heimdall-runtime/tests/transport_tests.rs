...
fn permissive_pipeline() -> Arc<AdmissionPipeline> {
    ...
    let allow_all = CompiledAcl::new(vec![AclRule { matchers: vec![], action: AclAction::Allow }]);
...
fn backpressure_udp_acl_deny_maps_to_silent_drop() {
    ...
    assert_eq!(udp_backpressure(&decision), BackpressureAction::UdpSilentDrop);
}
...
async fn udp_edns_payload_size_is_negotiated() {
    ...
    assert!(n <= 512, "response datagram must not exceed client-advertised EDNS size ({n} bytes)");

    // OPT RR must be present and reflect the negotiated size.
    let opt = resp
        .additional
        .iter()
        .find_map(|r| if let RData::Opt(o) = &r.rdata { Some(o) } else { None })
...
async fn udp_malformed_datagram_is_dropped_silently() {
    ...
    client.send_to(b"\xFF\xFF\xFF\xFF garbage", server_addr).await.unwrap();

    let mut buf = vec![0u8; 4096];
    let result =
        tokio::time::timeout(Duration::from_millis(200), client.recv_from(&mut buf)).await;
    assert!(result.is_err(), "no response expected for malformed datagram (timed out as expected)");
...
async fn udp_cookie_round_trip() {
    ...
    let opt_rr = OptRr {
        ...
        dnssec_ok: false,
        z: 0,
        options: vec![EdnsOption::Cookie(EdnsCookie { client: CLIENT_COOKIE, server: None })],
    };
    let mut hdr = Header::default();
    hdr.id = 0x5555;
    hdr.qdcount = 1;
    ...
    let cookie = opt
        .options
        .iter()
        .find_map(|o| if let EdnsOption::Cookie(c) = o { Some(c) } else { None })
        .expect("Cookie option must be present in response");

    assert_eq!(cookie.client, CLIENT_COOKIE, "client cookie must be echoed unchanged");
    let server_cookie = cookie.server.as_ref().expect("server cookie must be present");
...
async fn tcp_pipelining_limit_closes_after_max_queries() {
    ...
    assert_eq!(received, MAX_PIPELINE, "must receive {MAX_PIPELINE} responses");
...
async fn tcp_response_includes_keepalive_when_client_requests_it() {
    ...
    let has_keepalive =
        opt.options.iter().any(|o| matches!(o, EdnsOption::TcpKeepalive(Some(_))));
    assert!(has_keepalive, "response must carry TcpKeepalive option with a value");
